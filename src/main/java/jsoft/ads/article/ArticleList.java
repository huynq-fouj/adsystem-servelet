...
public class ArticleList extends HttpServlet {
	...
	protected void view(HttpServletRequest request, HttpServletResponse response, UserObject user) throws ServletException, IOException {
		...
		ArrayList<String> viewList = sc.viewArticle(new Triplet<>(null, page, (byte) 20));
		sc.releaseConnection();
		RequestDispatcher header = request.getRequestDispatcher("/header?pos=arlist");		
		if(header != null) {
		...
		out.append("<h1>Danh sách bài viết & tin tức</h1>");
